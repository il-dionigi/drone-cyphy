...
def run_sequence(scf, parameters):
    ...
    if verse > 0:
        # 1
        v = [xspeed, 0, 0]
        go_straight(cf, v, 3, fsi, z)

        # 2
        ztemp = z2
        go_vertical(cf, 1, fsi, ztemp, base, 1)

        # 3
        v = [-xspeed, 0, 0]
        go_straight(cf, v, 3, fsi, ztemp)
        ...
        # 7
        go_circular(cf, 180, 1, ztemp, 1, 2, fsi)
    ...
    go_circular(cf, 360, 1.5, ztemp, 1, 2, fsi)

    poshold(cf, 3, ztemp)

    # GO DOWN in 1 seconds
    timespan = 2
    go_vertical(cf, timespan, fsi, ztemp, base, -1)
